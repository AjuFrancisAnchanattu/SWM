...
//			secMax  = o.secondMax - (o.secondMax % o.stepSecond),
	...
	_onTimeChange: function(force) {
		...
		if (hasChanged) {
			...
			// setTimeout(this._updateDateTime(), 5000); 
//			setTimeout(function(){
//				  this._updateDateTime();
//				},2000); 
...
$.datepicker._gotoToday = function(id) {
	...
	// Rob M - added custom code to deal with timezones - ensures the "now" button takes the user to their local current date/time	
	var now = new Date(); 
  	var hour = 60*60*1000; 
  	var min = 60*1000; 
	...
	this._setTime(this._getInst($(id)[0]), new Date(now.getTime() + (now.getTimezoneOffset() * min) + (gmt_offset * hour)));
};
