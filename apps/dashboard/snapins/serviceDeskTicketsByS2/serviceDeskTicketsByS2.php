...
{	
	...
	public function generateSupportTicketsByS2()
	{
		...
		$datasetGlob = mysql::getInstance()->selectDatabase("serviceDesk")->Execute("SELECT DISTINCT(s2) AS name, (SELECT count(s2) FROM serviceDesk WHERE s2 = name) AS data FROM serviceDesk WHERE startDate BETWEEN '" . $this->yearFrom . "-" . $this->monthFrom . "-" . date("d") . "' AND '" . date("Y") . "-" . date("m") . "-31' GROUP BY s2");
