...
/**
 * DDDP All Chart Snapin for Dashboard
...
//		}
	...
	private function getCLIPRLIP($shippingPointsID)
	{
		...
		// ******************************** CLIP (Previous Month) ********************************
		$CLIPval = $this->currentMonthCountToDayNo;

		while($CLIPval <= 31)
		{
			$CLIPisReached = 0;

			$CLIPdataset = mysql::getInstance()->selectDatabase("SAP")->Execute("SELECT actualDate, agreedDate, aoIndent FROM dddp WHERE `actualDate` = '" . $this->previousMonthCountToYearNo . "-" . $this->previousMonthCountToMonthNo . "-" . $CLIPval . "' AND shippingPoint IN(" . $shippingPointsID .  ")" . $this->dddpLib->businessUnit);	
		...
		while($CLIPval <= $this->currentMonthCountToDayNo)
		{
			...
			$CLIPdataset = mysql::getInstance()->selectDatabase("SAP")->Execute("SELECT actualDate, agreedDate, aoIndent FROM dddp WHERE `actualDate` = '" . $this->currentMonthCountToYearNo . "-" . $this->currentMonthCountToMonthNo . "-" . $CLIPval . "' AND shippingPoint IN(" . $shippingPointsID .  ")" . $this->dddpLib->businessUnit);	
		...
		// ******************************** RLIP (Previous Month) ********************************
		$RLIPval = $this->currentMonthCountToDayNo;;

		while($RLIPval <= 31)
		{
			$RLIPisReached = 0;

			$RLIPdataset = mysql::getInstance()->selectDatabase("SAP")->Execute("SELECT actualDate, requiredDate, roIdent FROM dddp WHERE `actualDate` = '" . $this->previousMonthCountToYearNo . "-" . $this->previousMonthCountToMonthNo . "-" . $RLIPval . "' AND shippingPoint IN(" . $shippingPointsID .  ") AND defaultGIDate IS NULL" . $this->dddpLib->businessUnit);	
		...
		while($RLIPval <= $this->currentMonthCountToDayNo)
		{
			...
			$RLIPdataset = mysql::getInstance()->selectDatabase("SAP")->Execute("SELECT actualDate, requiredDate, roIdent FROM dddp WHERE `actualDate` = '" . $this->currentMonthCountToYearNo . "-" . $this->currentMonthCountToMonthNo . "-" . $RLIPval . "' AND shippingPoint IN(" . $shippingPointsID .  ") AND defaultGIDate IS NULL" . $this->dddpLib->businessUnit);	
