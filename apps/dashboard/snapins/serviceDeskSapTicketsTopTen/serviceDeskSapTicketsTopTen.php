...
class serviceDeskSapTicketsTopTen extends snapin
{
	...
	public function generateServiceDeskSapTicketsTopTen()
	{
		...
		$datasetGlob = mysql::getInstance()->selectDatabase("serviceDesk")->Execute("SELECT owner AS name, count(id) as data FROM serviceDesk WHERE s1='SAP' AND startDate BETWEEN '" . $this->currentYear . "-" . $this->lastMonth . "-1' AND '" .$this->currentYear . "-" . $this->lastMonth . "-" .$this->daysInMonth ."' GROUP BY owner LIMIT 10");
