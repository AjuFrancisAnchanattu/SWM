...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="serviceDeskSeverityByMonth">
				...
				<xsl:choose>
					<xsl:when test="allowed='1'">
						...
						<script type="text/javascript">
					        ...
					        var <xsl:value-of select="chartName" /> = new FusionCharts("../../lib/charts/FusionCharts/Column2D.swf", "<xsl:value-of select="chartName" />", screenW, "<xsl:value-of select="chartHeight" />");
					...
					<xsl:otherwise>
						iScapa You do not have access to the <xsl:value-of select="chartName" /> report.
					</xsl:otherwise>
				</xsl:choose>
