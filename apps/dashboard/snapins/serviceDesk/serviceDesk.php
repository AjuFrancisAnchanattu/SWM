...
class serviceDesk extends snapin
{
	...
	public function generateSupportTicketsMonthlyCharts()
	{
		...
		$datasetGlob = mysql::getInstance()->selectDatabase("serviceDesk")->Execute("SELECT owner AS name, count(id) as data FROM serviceDesk WHERE startDate BETWEEN '" . $this->yearFrom . "-" . $this->monthFrom . "-" . date("d") . "' AND '" . date("Y") . "-" . date("m") . "-31' GROUP BY owner");
