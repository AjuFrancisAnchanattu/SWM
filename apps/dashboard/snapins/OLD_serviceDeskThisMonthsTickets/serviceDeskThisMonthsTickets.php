...
{	
	...
	public function generateServiceDeskThisMonthsTickets()
	{
		...
		for ($i = 1; $i<=date("t"); $i++)
		{
			$dataset = mysql::getInstance()->selectDatabase("serviceDesk")->Execute("SELECT id FROM serviceDesk WHERE ticketType !='task' AND DAY(startDate)=" . $i . " AND MONTH(startDate)=" . date("n"));
