...
class dateCalcs extends page
{
	...
	{	
		...
		if (date("Y-m-d", mktime(0,0,0,$month,date("d"),$year)) == $this->todaysDate)
		{
			// set date to today			
		...
		else 
		{
			...
			$this->date = $this->endOfFiscalPeriod(date($year . "-" . $month . "-" . "15"));
		}
					
		//$this->date = "2010-05-10";
	 ...
	 */
	public function dateToRollingMonth() 
	{
		...
		{
			// calculates the rolling month up to the current date
	...
	public function dateToFiscalPeriod($date)
	{
		$sql = "SELECT min(fromDate) as startDate, period FROM `fiscalCalendar` WHERE toDate >= '" . $date . "' GROUP BY period";
	...
	/**
	 * Returns the end of a fiscal period for a specified date
	 *
	 * @param date $date
	 * @return date $fiscalStart
	 */
