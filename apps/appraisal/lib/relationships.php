 ...
 *
 * This is the relationships class.  This class is used to conduct the relationships part of the appraisal process.
 * 
 * @package apps	
 ...
 * @version 25/11/2008
 */
class relationships extends appraisalProcess 
{		
	/**
	...
	function __construct($appraisal)
	{
		...
		if (isset($_SESSION['apps'][$GLOBALS['app']]['relationships']['loadedFromDatabase']))
		{
	...
	public function load($id)
	{
		...
		$dataset = mysql::getInstance()->selectDatabase("appraisals")->Execute("SELECT * FROM relationships INNER JOIN appraisal ON relationships.appraisalId=appraisal.id WHERE appraisalId = "  . $id);

		if (mysql_num_rows($dataset) == 1)
		{
			
			$this->loadedFromDatabase = true;
			$_SESSION['apps'][$GLOBALS['app']]['relationships']['loadedFromDatabase'] = true;
		...
		{
			unset($_SESSION['apps'][$GLOBALS['app']]['relationships']);
			return false;
		}
	...
	{	
		...
		{			
			...
			$this->addLog("relationships report updated");
						
			// update
			mysql::getInstance()->selectDatabase("appraisals")->Execute("UPDATE relationships " . $this->form->generateUpdateQuery("relationships") . " WHERE appraisalId='" . $this->getappraisalId() . "'");
		...
		{			
			/* WC EDIT */
			$doUpdate = mysql::getInstance()->selectDatabase("appraisals")->Execute("SELECT count(appraisalId) as doUpdate FROM relationships WHERE appraisalId = '" . $this->getappraisalId() . "'");
			$doUpdateFields = mysql_fetch_array($doUpdate);
			if($doUpdateFields["doUpdate"]>0)
			{
				mysql::getInstance()->selectDatabase("appraisals")->Execute("UPDATE relationships " . $this->form->generateUpdateQuery("relationships")." WHERE appraisalId = '".$this->getappraisalId()."'");
			}else{
				mysql::getInstance()->selectDatabase("appraisals")->Execute("INSERT INTO relationships " . $this->form->generateInsertQuery("relationships"));
			}
			/* WC END */
	...
	{	
		...
		$this->form = new form("relationships");
		$this->form->setStoreInSession(true);
		...
		if(isset($_GET["print"]) && !isset($_REQUEST["printAll"])){//this means we are coming from the print function defined on homepage
			...
			$dataset2 = mysql::getInstance()->selectDatabase("appraisals")->Execute("SELECT * FROM relationships LEFT JOIN appraisal ON relationships.appraisalId=appraisal.id WHERE appraisalId = '"  . $this->appraisal->getId()."'");
		...
		$appraisalId->setTable("relationships");
		...
		$emailMessage->setTable("relationships");
		...
		$processOwner3->setTable("relationships");
