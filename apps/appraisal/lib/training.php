 ...
 *
 * This is the training class.  This class is used to conduct the training part of the appraisal process.
 * 
 * @package apps	
 ...
 * @version 25/11/2008
 */
class training extends appraisalProcess 
{		
	/**
	...
	function __construct($appraisal)
	{
		...
		if (isset($_SESSION['apps'][$GLOBALS['app']]['training']['loadedFromDatabase']))
		{
	...
	public function load($id)
	{
		...
		$dataset = mysql::getInstance()->selectDatabase("appraisals")->Execute("SELECT * FROM training INNER JOIN appraisal ON training.appraisalId=appraisal.id WHERE appraisalId = "  . $id);

		if (mysql_num_rows($dataset) == 1)
		{
			
			$this->loadedFromDatabase = true;
			$_SESSION['apps'][$GLOBALS['app']]['training']['loadedFromDatabase'] = true;
		...
		{
			unset($_SESSION['apps'][$GLOBALS['app']]['training']);
			return false;
		}
	...
	{	
		...
		{			
			...
			$this->addLog("training report updated");
						
			// update
			mysql::getInstance()->selectDatabase("appraisals")->Execute("UPDATE training " . $this->form->generateUpdateQuery("training") . " WHERE appraisalId='" . $this->getappraisalId() . "'");
		...
		{			
			/* WC EDIT */
			$doUpdate = mysql::getInstance()->selectDatabase("appraisals")->Execute("SELECT count(appraisalId) as doUpdate FROM training WHERE appraisalId = '" . $this->getappraisalId() . "'");
			$doUpdateFields = mysql_fetch_array($doUpdate);
			if($doUpdateFields["doUpdate"]>0)
			{
				mysql::getInstance()->selectDatabase("appraisals")->Execute("UPDATE training " . $this->form->generateUpdateQuery("training")." WHERE appraisalId = '".$this->getappraisalId()."'");
			}else{
				mysql::getInstance()->selectDatabase("appraisals")->Execute("INSERT INTO training " . $this->form->generateInsertQuery("training"));
			}
			/* WC END */
	...
	{	
		...
		if(isset($_GET["print"]) && !isset($_REQUEST["printAll"])){//this means we are coming from the print function defined on homepage
			...
			$dataset2 = mysql::getInstance()->selectDatabase("appraisals")->Execute("SELECT * FROM training LEFT JOIN appraisal ON training.appraisalId=appraisal.id WHERE appraisalId = '"  . $this->appraisal->getId()."'");
		...
		$emailMessage->setTable("training");
		...
		$processOwner3->setTable("training");
