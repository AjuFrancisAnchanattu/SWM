...
class customerPopup
{
	private $sapNumber = null;

	private $xml = "";

	function __construct()
	{
		if( isset($_GET['sapNumber']) && sapCustomer::isSapNumberValid($_GET['sapNumber']) )
		{
			$this->sapNumber = $_GET['sapNumber'];
			$this->xml = "<sapNumber>" . $this->sapNumber . "</sapNumber>";
			$this->xml.= "<sapName>" . sapCustomer::getName( $this->sapNumber ) . "</sapName>";

			$this->xml .= $this->getComplaints();
		}
		else
		{
			$this->xml = "<wrongSapNumber />";
		}
	...
	private function getComplaints()
	{
		$sql = "SELECT *
				FROM complaint
				WHERE sapCustomerNo ='" . $this->sapNumber . "' 
				AND submitStatus = 1 
	...
	public function output()
	{
		...
		$final .= "<customerPopup>" . $this->xml . "</customerPopup>";
        ...
        // load xsl
        $xsl = new DomDocument;
        $xsl->load("./apps/customerComplaints/customerPopup/customerPopup.xsl");
