...
<xsl:template match="customerPopup">
	<html>
		<head>
			<title>{TRANSLATE:customerPopup_title}</title>
			<style type="text/css">
				body
				{
		...
		<body>
			<div id="content">
				<div id="title">
					<b>{TRANSLATE:all_compalints_for_customer}:</b> <span style="margin-left: 10px"><xsl:value-of select="sapNumber"/></span>
				</div>
				<div id="top_info">
					<b>{TRANSLATE:customer_name}:</b> <span style="margin-left: 10px; font-style: italic;"><xsl:value-of select="sapName" /></span>
				</div>
