...
class chart_ByNumber_ByMonth_ForSite extends dashboardsLib
{
	public function getChartXML($args)
	{
		$siteName = $args;
		$xml = "<chart caption='Complaints Number by Month (Customer Complaints) 'subcaption='For $siteName' labelDisplay='stagger' showValues='1' decimals='1' useRoundEdges='1' exportEnabled='1' exportAtClient='1' exportHandler='ccChart_openClosed_Exporter' registerWithJS='1' exportFileName='ccChart_ByNumber_$siteName' >";
		...
		foreach( $fiscal as $period )
		{
			...
			$count_opened = $this->getInvoicesCountForPeriod_opened($period, $siteName);
			$count_remained = $this->getInvoicesCountForPeriod_remained($period, $siteName);
			$count_valid = $this->getInvoicesCountForPeriod_valid($period, $siteName);
			
			$tooltext = "$count_opened have been opened in $month_long $year in $siteName";
			$xml_opened .= "<set value='$count_opened' toolText='$tooltext' />";
			
			$tooltext = "$count_remained remained opened in $month_long $year in $siteName";
			$xml_remained .= "<set value='$count_remained' toolText='$tooltext' />";
			
			$tooltext = "$count_valid from $month_long $year were valid in $siteName";
	...
	private function getInvoicesCountForPeriod_opened($period, $siteName)
	{
		$count = 0;
	...
	private function getInvoicesCountForPeriod_remained($period, $siteName)
	{
		$count = 0;
		$date_from = $period["fromDate"];
		$date_to = $period["toDate"];
	...
	private function getInvoicesCountForPeriod_valid($period, $siteName)
	{
		$count = 0;
		$date_from = $period["fromDate"];
		$date_to = $period["toDate"];
