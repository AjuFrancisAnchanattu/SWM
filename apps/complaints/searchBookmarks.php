...
class searchBookmarks extends page
{
	...
	function __construct()
	{
		...
		$this->defineColumns();
		*/
		if (isset($_REQUEST['action']) && ($_REQUEST['action'] == 'bookmark' || $_REQUEST['action'] == 'view'))
		{
			...
			$blah = unserialize($fieldsBookmark['filters']);

			$this->selectedFilters = $blah;
			//$_SESSION["selectedFilters"] = $this->selectedFilters;
			/*echo "<pre>";
			print_r($this->selectedFilters);
			echo "</pre>";
			exit;*/
		}
		...
		if (isset($_REQUEST['action']) && ($_REQUEST['action'] == 'bookmark' || $_REQUEST['action'] == 'view'))
		{
			...
			if($fieldsBookmark['columns']){
				$cols = unserialize($fieldsBookmark['columns']);
			}
			/*echo "<pre>";
			print_r($cols);
			echo "</pre>";
			...
			//exit;
			//var_dump($_SESSION['apps'][$GLOBALS['app']]["selectedFilters"]);
			/*WC.AE 16/01/08
				removed this whole block of code
				seemed to be causing the inability to press back button
			*/
			/*
			if (!isset($_SESSION['apps'][$GLOBALS['app']]["selectedFilters"]))
			{
				//WC.GB 14/11/07
				//added code to see what's breaking 'back' functionality of
				//report links and why bookmarks won't work from anywhere other
				//than search page.

					//echo 'system forward';
					//die;
				//if($_REQUEST['bookmarkId']){
					//$this->redirect("searchBookmarks?action=bookmark&bookmarkId=".$_REQUEST['bookmarkId']);
					//exit;
				//}
				//WC.GB END 14/11/07
			...
			*/
			...
			/* WC.AE 16/01/08
				put the for loop inside if(isset in order to remove errors on frontend
			*/
			if (isset($_SESSION['apps'][$GLOBALS['app']]["selectedFilters"])){
			...
			/* WC.AE END */
			/* WC.AE 18/01/08*/
			unset($_SESSION["searchColumns"]);
			$_SESSION["searchColumns"] = $cols;
			$_SESSION["reportType"] = $fieldsBookmark["reportType"];
			...
			{
				$this->chooseReportForm->get("reportType")->setValue($fieldsBookmark['reportType']);
		...
		$this->output('./apps/complaints/xsl/searchBookmarks.xsl');
	}
...
// Conclusion Table 24 fields now!
				...
				if(in_array("sp_supplierCreditNote", $this->selectedColumns) || $this->showAllCols)
				{
					$results->addColumn(new column("conclusion.`sp_supplierCreditNumber_quantity`", "sp_supplierCreditNumber_quantity", "sp_supplierCreditNote_value", true));
...
*/				break;
			...
			case 'customQuality':
				...
				if(in_array("problem_description", $this->selectedColumns) || $this->showAllCols)
					$results->addColumn(new column("complaint.`problemDescription`", "problemDescription", "problem_description", true));
				//if(in_array("containment_action", $this->selectedColumns) || $this->showAllCols)
					//$results->addColumn(new column("complaint.`containmentAction`", "containmentAction", "containment_action", true));
				if(in_array("requestedAction", $this->selectedColumns) || $this->showAllCols)
					$results->addColumn(new column("complaint.`requestedAction`", "requestedAction", "requested_action", true));
				if(in_array("materialInvolved", $this->selectedColumns) || $this->showAllCols)
					$results->addColumn(new column("complaint.`qu_materialInvolved`", "qu_materialInvolved", "material_involved", true));
...
FROM report r GROUP BY owner*/
	...
	public function updatePerformanceValues()
	{
		...
		while ($row = mysql_fetch_array($dataset)) {
			...
			$performance_3d = $this->datediff($row['customerComplaintDate'], $row['openDate']);
		...
		while ($row = mysql_fetch_array($dataset)) {
			...
			mysql::getInstance()->selectDatabase("complaints")->Execute("UPDATE `complaint` SET `performance_cco` = '" . $performance_cco . "' WHERE id = " . $row['id'] . "");
...
class complaintDateColumn extends column
{
	...
	{
		$xml = "<searchColumn sortable=\"" . $this->getSortable() . "\"><text>\n";
		$xml .= page::transformDateForPHP($fields[$this->getName()]) == "30/11/1999" ? $fields[$this->getName()] = "" : page::transformDateForPHP($fields[$this->getName()]);
		$xml .= "</text></searchColumn>";
...
class complaintHowLongColumn extends column
{
	public function getOutput($fields)
	{
		...
		//$xml .= page::transformDateTimeForPHP($fieldsActionLog['actionDate']) . "<br />";
