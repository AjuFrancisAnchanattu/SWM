...
class generateSupplier8dit extends page
{	
	...
	public function generateWordDocument()
	{
		...
		$file = './apps/complaints/word/supplier8d-itTemplate.rtf';
...
{\*\formfield{\fftype1\ffres25\fftypetxt0\ffhps20{\*\ffname Check1}\ffdefres1}}}}{\fldrslt }}{\fs18\lang2057\langfe1033\langnp2057\insrsid7735582 {\*\bkmkend Check1}";		
		...
		$supplierSAPNumber = $fields['sp_sapSupplierNumber'];
		...
		$materialGroup = $fields['sapMaterialGroup'];
		$sapItemNumbers = $fields['sapItemNumbers'];
		$quantity = $fields['quantityUnderComplaint_quantity'] . $fields['quantityUnderComplaint_measurement'];
		...
		$data = str_replace('[[COL]]',replaceForeignCharsForRTF($colour),$data);
		$data = str_replace('[[SIN]]',replaceForeignCharsForRTF($sapItemNumbers),$data);
		$data = str_replace('[[MG]]',replaceForeignCharsForRTF($materialGroup),$data);
		$data = str_replace('[[SIN]]',replaceForeignCharsForRTF($scapa),$data);
		...
		$data = str_replace('[[COM]]',replaceForeignCharsForRTF($comments),$data);	
		...
		$data = str_replace('[[SCN]]',replaceForeignCharsForRTF($supplierSAPNumber),$data);
		...
		// Save the file here
		$fpSaveFile = './apps/complaints/word/files/supplier8d-it' . $fields['id'] .  '.rtf';				
		...
		chmod("./apps/complaints/word/files/supplier8d-it" . $fields['id'] .  ".rtf", 0777);
		...
		mysql::getInstance()->selectDatabase("complaints")->Execute("INSERT INTO `documents` (complaintId, type, date, language) VALUES(" . $fields['id'] . ", 'supplier8d', '" . common::nowDateForMysql() . "', 'it')");
