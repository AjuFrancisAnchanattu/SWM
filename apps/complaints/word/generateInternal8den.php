...
class generateInternal8den extends page
{	
...
{\*\formfield{\fftype1\ffres25\fftypetxt0\ffhps20{\*\ffname Check1}\ffdefres1}}}}{\fldrslt }}{\fs18\lang2057\langfe1033\langnp2057\insrsid7735582 {\*\bkmkend Check1}";
		...
		$file = './apps/complaints/word/internal8d-enTemplate.rtf';
		...
		$fields['dateOfManufacturing'] == "0000-00-00" ? $manufacturingDate = "" : $manufacturingDate = common::transformDateForPHP($fields['dateOfManufacturing']);
		...
		elseif ($cType == "quality_complaint")
		{
			$complaintType = "I";
		}
		...
		$requestActions = $fields['requestedAction'];
		...
		$comments = $fieldsEval['additionalComments'];
		...
		$scRef = $scRefValue;
		$scDate = $scDateValue;
		$depOfError = $fields['sp_siteConcerned'];
		
		// Added 18/12/2008
		$internalRefNumber = $fields['internalReferenceNumber'];
		$lotNo = $fields['lotNo'];
		$manufacturingNumber = $fields['manufacturingNumber'];
		$whereErrorOccured = $fields['whereErrorOccured'];
		$foundBy = $fields['qu_foundBy'];
		$supplierBatchNumber = $fields['supplierBatchNumber'];
		$howErrorDetected = $fields['howErrorDetected'];
		...
		$reworkTheGoods = $fieldsEval['qu_reworkGoods'] == "YES" ? "$tickBox " : "$blankBox";
		$disposeTheGoods = $fieldsEval['disposeGoods'] == "YES" ? "$tickBox " : "$blankBox";
		$useTheGoods = $fieldsEval['qu_useGoods'] == "YES" ? "$tickBox " : "$blankBox";
		...
		$data = str_replace('[[IRN]]',replaceForeignCharsForRTF($internalRefNumber),$data);
		$data = str_replace('[[MNO]]',replaceForeignCharsForRTF($manufacturingNumber),$data);
		$data = str_replace('[[WEO]]',replaceForeignCharsForRTF($whereErrorOccured),$data);
		$data = str_replace('[[HED]]',replaceForeignCharsForRTF($howErrorDetected),$data);
		$data = str_replace('[[SUPBN]]',replaceForeignCharsForRTF($supplierBatchNumber),$data);
		$data = str_replace('[[MDATE]]',replaceForeignCharsForRTF($manufacturingDate),$data);
		$data = str_replace('[[LNO]]',replaceForeignCharsForRTF($lotNo),$data);
		$data = str_replace('[[FB]]',replaceForeignCharsForRTF($foundBy),$data);
		...
		$data = str_replace('[[RTG]]',replaceForeignCharsForRTF($reworkTheGoods),$data);
		$data = str_replace('[[DTG]]',replaceForeignCharsForRTF($disposeTheGoods),$data);
		$data = str_replace('[[UTG]]',replaceForeignCharsForRTF($useTheGoods),$data);
		...
		$data = str_replace('[[DEPOFERR]]',replaceForeignCharsForRTF($depOfError),$data);
		...
		// Save the file here
		$fpSaveFile = './apps/complaints/word/files/internal8d-en' . $fields['id'] .  '.rtf';
		...
		chmod("./apps/complaints/word/files/internal8d-en" . $fields['id'] .  ".rtf", 0777);
		
		$this->addLog("8D Created");
		
		mysql::getInstance()->selectDatabase("complaints")->Execute("DELETE FROM `documents` WHERE complaintId='" . $fields['id'] . "' AND type = 'internal8d'");	
		mysql::getInstance()->selectDatabase("complaints")->Execute("INSERT INTO `documents` (complaintId, type, date, language) VALUES(" . $fields['id'] . ", 'internal8d', '" . common::nowDateForMysql() . "', 'en')");
