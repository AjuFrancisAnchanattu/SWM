...
class newProcessOwner3 extends page 
{
	function __construct()
	{
		parent::__construct();
		
		if (!isset($_REQUEST['processOwner3']))
		{
			die();
		}
		
		$dataset = mysql::getInstance()->selectDatabase("MEMBERSHIP")->Execute("SELECT * FROM employee WHERE (`firstName` LIKE '" . $_REQUEST['processOwner3'] . "%') OR (`lastName` LIKE '" . $_REQUEST['processOwner3'] . "%')ORDER BY `firstname` LIMIT 20");
