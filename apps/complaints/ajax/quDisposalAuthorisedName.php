...
class quDisposalAuthorisedName extends page 
{
	function __construct()
	{
		parent::__construct();
		
		if (!isset($_REQUEST['qu_disposalAuthorisedName']))
		{
			die();
		}
		
		$dataset = mysql::getInstance()->selectDatabase("MEMBERSHIP")->Execute("SELECT * FROM employee WHERE (`firstName` LIKE '" . $_REQUEST['qu_disposalAuthorisedName'] . "%') ORDER BY `firstname` LIMIT 20");
