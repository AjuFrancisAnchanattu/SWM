...
class processOwnerRequest extends page 
{
	function __construct()
	{
		parent::__construct();
		
		if (!isset($_REQUEST['processOwnerRequest']))
		{
			die();
		}
		
		$dataset = mysql::getInstance()->selectDatabase("MEMBERSHIP")->Execute("SELECT * FROM employee WHERE (`firstName` LIKE '" . $_REQUEST['processOwnerRequest'] . "%') OR (`lastName` LIKE '%" . $_REQUEST['processOwnerRequest'] . "%') ORDER BY `firstname` LIMIT 20");
