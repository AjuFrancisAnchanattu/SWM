...
class processOwner3Request extends page 
{
	function __construct()
	{
		parent::__construct();
		
		if (!isset($_REQUEST['processOwner3Request']))
		{
			die();
		}
		
		$dataset = mysql::getInstance()->selectDatabase("MEMBERSHIP")->Execute("SELECT * FROM employee WHERE (`firstName` LIKE '" . $_REQUEST['processOwner3Request'] . "%') OR (`lastName` LIKE '%" . $_REQUEST['processOwner3Request'] . "%') ORDER BY `firstname` LIMIT 20");
