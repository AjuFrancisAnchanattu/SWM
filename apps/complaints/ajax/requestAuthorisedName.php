...
class requestAuthorisedName extends page 
{
	function __construct()
	{
		parent::__construct();
		
		if (!isset($_REQUEST['sp_requestAuthorisedName']))
		{
			die();
		}
		
		$dataset = mysql::getInstance()->selectDatabase("MEMBERSHIP")->Execute("SELECT * FROM employee WHERE (`firstName` LIKE '" . $_REQUEST['sp_requestAuthorisedName'] . "%') ORDER BY `firstname` LIMIT 20");
