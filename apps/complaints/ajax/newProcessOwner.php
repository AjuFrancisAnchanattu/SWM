...
class newProcessOwner extends page 
{
	function __construct()
	{
		...
		//$dataset = mysql::getInstance()->selectDatabase("MEMBERSHIP")->Execute("SELECT * FROM employee WHERE (`firstName` LIKE '" . $_REQUEST['processOwner'] . "%') OR (`lastName` LIKE '" . $_REQUEST['processOwner'] . "%')ORDER BY `firstname` LIMIT 20");
		
		$dataset = mysql::getInstance()->selectDatabase("MEMBERSHIP")->Execute("SELECT * FROM employee WHERE (`firstName` LIKE '" . $_REQUEST['processOwner'] . "%') OR (`lastName` LIKE '%" . $_REQUEST['processOwner'] . "%') OR (CONCAT(firstName, ' ', lastName) LIKE '" . $_REQUEST['processOwner'] . "%') ORDER BY `firstname` LIMIT 20");
