...
class scapaSalesName extends page 
{
	function __construct()
	{
		parent::__construct();
		
		if (!isset($_REQUEST['scapaSalesName']))
		{
			die();
		}
		
		$dataset = mysql::getInstance()->selectDatabase("MEMBERSHIP")->Execute("SELECT * FROM employee WHERE (`firstName` LIKE '" . $_REQUEST['scapaSalesName'] . "%') OR (`lastName` LIKE '%" . $_REQUEST['scapaSalesName'] . "%') ORDER BY `firstname` LIMIT 20");
