...
class reportautocomplete extends page 
{
	function __construct()
	{
		...
		{
			$dataset = mysql::getInstance()->selectDatabase("comms")->Execute("SELECT * FROM comm WHERE (subject LIKE '%" . $_REQUEST['commsId'] . "%') ORDER BY id");
		}
		else 
		{
			$dataset = mysql::getInstance()->selectDatabase("comms")->Execute("SELECT * FROM comm WHERE (subject LIKE '%" . $_REQUEST['commsId'] . "%') AND newsType = 1 ORDER BY id");
		}
