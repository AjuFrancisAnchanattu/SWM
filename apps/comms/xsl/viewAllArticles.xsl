...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="article">
		<a href="./viewArticle?id={articleId}"><tr 
		...
		>
			<td style="padding: 5px;"><strong><xsl:value-of select="articleTitle" /></strong></td>
			<td><p><xsl:value-of select="articleBody" />...</p></td>
			<td><em><xsl:value-of select="articleDate" /></em></td>
			<xsl:if test="../commAdmin='true'">
				<td>
					<xsl:choose>
