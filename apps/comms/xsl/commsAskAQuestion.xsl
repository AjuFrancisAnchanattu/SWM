...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="commsHomeAskAQuestion">
	...
	<xsl:template match="comms_report">
		...
		<h1 style="margin-bottom: 10px;">{TRANSLATE:question_id}: 
	...
	<xsl:template match="commsSummary">
		...
		<table width="100%" cellspacing="0" cellpadding="4" class="indented">
			...
			<tr class="valid_row">
				...
				<td class="valid_row"><a href="viewAskAQuestion?id={commId}"><strong>{TRANSLATE:view}</strong></a><xsl:if test="../../commAdmin='true'"> | <a href="resumeAskAQuestion?commId={commId}&amp;type=askAQuestion"><strong>{TRANSLATE:edit}</strong></a> | </xsl:if>
				
				<xsl:if test="../../commAdmin='true'">
					<xsl:choose>
						<xsl:when test="published='0'">
							<a href="tasks?commId={commId}&amp;task=publishQuestion"><strong>{TRANSLATE:publish_news}</strong></a> 
						</xsl:when>
						<xsl:otherwise>
							<a href="tasks?commId={commId}&amp;task=unpublishQuestion"><strong>{TRANSLATE:unpublish_news}</strong></a>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:if>
	...
	<xsl:template match="sections">
		...
		<xsl:choose>
			<xsl:when test="admin='true'">
				<input type="submit" value="View" onclick="buttonLink('viewAskAQuestion?commId={@id}&amp;status=comm')" />	
				<input type="submit" value="Edit" onclick="buttonLink('resumeAskAQuestion?commId={@id}&amp;status=comm')" />
			</xsl:when>
