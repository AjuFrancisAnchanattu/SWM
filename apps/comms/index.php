...
class index extends page
{
	...
	function __construct()
	{
		...
		if (isset($_SESSION['apps'][$GLOBALS['app']]['id']) || isset($_REQUEST['id']))
		{
			...
			if ($this->comm->load($_POST['report']))
			{
				...
				*/
				...
				$dataset = mysql::getInstance()->selectDatabase("comms")->Execute("SELECT * FROM actionLog WHERE commId ='" . $_POST['report'] . "' AND type = 'news' ORDER BY actionDate DESC, commId DESC");
		...
		$this->output('./apps/comms/xsl/comms.xsl');
	}
