...
class view extends manipulate 
{
	function __construct()
	{
		...
		if(isset($_REQUEST['commId']) && $_REQUEST['commId'] != "")
		{
			$id = $_REQUEST['commId'];
		}
		...
		{
			$this->comm->load($id);		//load the npi from its ID
		}
		...
		$this->add_output($this->doStuffAndShow("readOnly"));		//chooses what should be displayed on the comm screen. i.e. what part of the comm process
		...
		$this->output('./apps/comms/xsl/viewNews.xsl');
	}	
}
