...
class materialgroup extends page 
{
	function __construct()
	{
		...
		if (!isset($_REQUEST['materialKey']))
		{
			die();
		}
		
		$dataset = mysql::getInstance()->selectDatabase("SAP")->Execute("SELECT * FROM material_group WHERE (`key` LIKE '" . $_REQUEST['materialKey'] . "%') ORDER BY `key` LIMIT 20");
