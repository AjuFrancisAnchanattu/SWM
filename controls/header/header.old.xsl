				...
				<table border="0" cellpadding="0" cellspacing="0" width="100%">
					<tr>
						<td rowspan="3">
							<table border="0" cellpadding="0" cellspacing="0" width="100%">
								<tr>
									<td><a href="/"><img src="/images/logo_new.jpg" class="logo" alt="Scapa Intranet" /></a></td>
						...
						<td>
							...
							<table cellpadding="0" cellspacing="0" width="550px" height="25px">
								<tr>
									<td>
										<strong>{TRANSLATE:scapa_instant_messaging}: </strong><img src="/images/icons2020/chat.jpg" hspace="5" />
									</td>
									...
									<xsl:choose>
										<xsl:when test="chat_id != 0">
											<td>
												<div align="right">
													<a href="#" onclick="window.open('/apps/chat/chat?person_name={myname}&amp;chat_name={chat_id}&amp;NTLogon={myNTLogon}&amp;myphoto={myphoto}', 'initiateChat','menubar=0,resizable=0,width=300,height=500');"><strong>{TRANSLATE:chat_requested}</strong></a> ({TRANSLATE:click_to_open} )
												</div>
											</td>
										</xsl:when>
										<xsl:otherwise>
...
-->							</table>
								...
								<div class="nav">
									<ul>
										...
										<xsl:for-each select="item">
											...
											<li id="li_toolbar_{@id}" onMouseOver="showDropdown('{@id}');" onMouseOut="hideDropdown('{@id}');"><a href="{@url}"><xsl:value-of select="@title"/></a></li>
	...
	<xsl:template match="child">
		<div class="dropdown_menu" id="dropdown_{@id}" onMouseOver="showSubDropdown('{@parent}', '{@id}');" onMouseOut="hideSubDropdown('{@id}', false);"><a href="{@url}"><xsl:value-of select="@title"/></a></div>
	</xsl:template>
	
	<xsl:template match="grandchild">
		
		<div class="dropdown_menu" id="dropdown_{@id}"><a href="{@url}"><xsl:value-of select="@title"/></a></div>
