...
class header extends control
{
	...
	public function output()
	{
		...
		$dataset = mysql::getInstance()->selectDatabase("membership")->Execute("SELECT NTLogon FROM employee WHERE LastActivity > '" . date("Y/m/d H:i:s", time()-600) . "' ORDER BY NTLogon ASC");
...
class headerMenuItem
{
	...
	public function output($parent = 0)
	{
		...
		{
			$output = sprintf('<%s title="%s" url="%s" id="%u" parent="%u">', $this->type, page::xmlentities(translate::getInstance()->translate($this->title)), $this->url, $this->id, $parent);
