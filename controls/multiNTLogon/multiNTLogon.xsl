...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="multiNTLogon">
		...
		<xsl:element name="input">
			...
			<xsl:attribute name="value">Edit users...</xsl:attribute>
<!--			<xsl:attribute name="onClick">open_NTLogon_window("<xsl:value-of select="name" />", document.getElementById(<xsl:value-of select="name" />).value);</xsl:attribute>
-->
			<xsl:attribute name="onClick">open_NTLogon_window("<xsl:value-of select="name" />", "<xsl:value-of select="value" />");</xsl:attribute>
		</xsl:element>
